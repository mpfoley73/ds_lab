...
subtitle: "What They Mean, How to Use Them"
date: 2023-09-14
...
- The joint distribution of $x$ and $y$ is $f(x, y) = f(x|y)f(y) = f(y|x)f(x)$ where $f(x|y)$ is called the _conditional PDF_.
...
data.frame(
  ...
  geom_text(
    aes(x = events, label = round(pmf, 3), y = pmf + 0.001),
  ...
  labs(
    title = "PMF and CDF of Binomial Distribution",
    subtitle = paste0(
      "Bin(",
      nrow(dat_bin),
      ", ",
      scales::comma(p, accuracy = .01),
      ")"
    ),
    x = "X = k",
    y = "Density"
